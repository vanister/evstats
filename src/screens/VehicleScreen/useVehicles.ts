...
export function useVehicles() {
  ...
  const loadVehicleStats = useCallback(async (): Promise<void> => {
  ...
  const addNewVehicle = useCallback(
    async (vehicle: Vehicle): Promise<string | null> => {
      try {
        ...
        return (
          error.message || 'Failed to add vehicle. Please check your information and try again.'
        );
      }
    },
    [vehicleService, dispatch]
